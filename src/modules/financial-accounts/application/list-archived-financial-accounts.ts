...
export type ListArchivedFinancialAccountsResult =
  | {
      success: true;
      financialAccounts: Awaited<ReturnType<typeof listArchivedFinancialAccountsByWorkspace>>;
    }
  | {
      success: false;
      message: string;
...
export async function listArchivedFinancialAccounts(
  workspaceId: string,
): Promise<ListArchivedFinancialAccountsResult> {
  ...
  try {
    const financialAccounts = await listArchivedFinancialAccountsByWorkspace(workspaceId);
