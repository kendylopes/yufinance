import "server-only";

import { getCurrentUser } from "@/modules/identity/application/get-current-user";
import { findWorkspaceMembership } from "@/modules/workspace/infrastructure/workspace-repository";

import { canManageCategories } from "../domain/category-permissions";
import { updateActiveCategoryRecord } from "../infrastructure/category-repository";
import { type UpdateCategoryInput, updateCategorySchema } from "./update-category.schema";

export type UpdateCategoryResult =
  | {
      success: true;
    }
  | {
      success: false;
      message: string;
    };

export async function updateCategory(input: {
  workspaceId: string;
  categoryId: string;
  data: UpdateCategoryInput;
}): Promise<UpdateCategoryResult> {
  const validation = updateCategorySchema.safeParse(input.data);

  if (!validation.success) {
    return {
      success: false,
      message: "Os dados informados não são válidos.",
    };
  }

  const currentUser = await getCurrentUser();

  if (!currentUser) {
    return {
      success: false,
      message: "Você precisa estar autenticado para continuar.",
    };
  }

  const membership = await findWorkspaceMembership(currentUser.id, input.workspaceId);

  if (!membership) {
    return {
      success: false,
      message: "Você não possui acesso a este espaço financeiro.",
    };
  }

  if (!canManageCategories(membership.role)) {
    return {
      success: false,
      message: "Você não possui permissão para editar categorias.",
    };
  }

  try {
    const category = await updateActiveCategoryRecord({
      workspaceId: input.workspaceId,
      categoryId: input.categoryId,
      category: validation.data,
    });

    if (!category) {
      return {
        success: false,
        message: "Categoria não encontrada.",
      };
    }

    return {
      success: true,
    };
  } catch {
    return {
      success: false,
      message: "Não foi possível atualizar a categoria. Tente novamente.",
    };
  }
}
