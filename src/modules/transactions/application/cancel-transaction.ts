...
export async function cancelTransaction(input: {
  ...
  if (!canManageTransactions(membership.role)) {
    return {
      success: false,
      message: "Você não possui permissão para cancelar transações.",
    };
  }

  try {
    const transaction = await cancelActiveTransactionRecord({
    ...
    if (!transaction) {
      return {
        success: false,
        message: "Transação ativa não encontrada.",
      };
    }
    ...
    return {
      success: false,
      message: "Não foi possível cancelar a transação. Tente novamente.",
    };
  }
}
